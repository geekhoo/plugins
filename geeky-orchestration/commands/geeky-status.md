...
$ARGUMENTS: a folder produced by `/geeky-plan`. If missing or the folder does not contain `kanban.md`, STOP and ask the user. This command is strictly read-only — never modify, commit, or delegate.
...
Fast situational awareness when resuming a planning/implementation session. Answers: where are we, what's blocked, what's deferred, what was the last thing done. Useful before invoking `/geeky-implement` to decide whether to resume, retarget, or hand-fix.
...
1. **Validate the folder.** Pick the runtime by host OS:
