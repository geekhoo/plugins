...
description: Execution phase of the geeky orchestration workflow. Drives a geeky-plan package (implementation-plan.md, kanban.md, tasks/Tx-*.md, handoff.md) through the kanban, delegating geeky-coder subagents, validating, running code + PM review, and committing per phase. Use when asked to implement/execute a planning package, run the kanban tasks, or orchestrate coders against a plan. Invoked by the /geeky-implement command and usable directly by any agent. Supports --phase, --dry-run, --serial flags.
---
