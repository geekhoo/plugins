...
## 2. Establish a clean environment first — invoke the matching preflight
Contract failures are often environment, not API, bugs. Run the preflight skill matching the stack: `env-check` for FastAPI/Python repos (`.venv`, deps, encoding, DB), **`dotnet-env-check` for ASP.NET repos** (SDK, restore, EF migrations per DbContext, DB reachability). Then:
...
## Related skills
- `env-check` — Python environment preflight (Section 2).
- `dotnet-env-check` — .NET environment preflight (Section 2).
