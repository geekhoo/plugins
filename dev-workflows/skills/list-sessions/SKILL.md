---
name: list-sessions
description: Use whenever the user asks to find, list, recall, or search PAST Claude Code sessions or conversations "across all repos/projects," "everywhere," or "across the whole machine" — including typo'd phrasings like "conversions" — or what's been worked on historically outside the current session's own context, or to spot-check whether a specific past conversation exists, on what date, or in which repo. Do NOT use this for "what did we just do" or "what happened earlier in this session" (already in context) or for git history questions (use git log) — this is for on-disk cross-session Claude Code transcript history.
---
