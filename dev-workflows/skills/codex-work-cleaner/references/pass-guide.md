# Codex Work Cleaner Pass Guide

Load this reference when a cleanup spans multiple files, generated docs, plugin or skill packages, packet artifacts, frontend surfaces, hooks, or runtime compatibility files.
...
- Skill text that tells Codex generic facts it already knows.

Keep comments that explain why: business rules, migrations, host quirks, validation gaps, compliance, performance, compatibility, or known traps.

## Pass 3: Generated Surface Tightening

Tighten code or docs produced by broad agents:

- Remove duplicated prose across `handoff.md`, `kanban.md`, task notes, and reports when one artifact is the source of truth.
...
- Trim over-specific examples that make plugin-eval or link checkers treat placeholders as live broken links.
...
Leave abstractions in place when they are public API, plugin compatibility surface, skill trigger metadata, framework convention, test seam, or documented future-extension point.

## Pass 5: Windows And Codex Environment Fit
...
- Keep `.codex` runtime-layer changes separate from durable plugin source changes under `C:\Users\gerald.khoo\geekhoo-plugins`.
