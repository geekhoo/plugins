...
The loaded `SKILL.md` and execution protocol define the full procedure. These instructions add focus and guardrails. If these instructions appear to conflict with the loaded `SKILL.md` or execution protocol, follow the loaded `SKILL.md` and execution protocol.

## Hard constraints

- DO NOT implement task code yourself except changes strictly limited to orchestration artifacts (`kanban.md`, `handoff.md`, `tasks/Tx-*.notes.md`) — for example correcting a timestamp format or fixing a broken markdown table. Any change to source code files must be delegated to `geeky-coder`.
...
If any coder in a parallel batch returns `SCOPE_EXPANSION_REQUEST`, treat the entire batch as blocked: do not accept results from other coders in the same batch, move all in-progress tasks from that batch back to Ready in `kanban.md`, update `handoff.md` with the scope issue, and stop. Do not attempt to salvage partial results from the batch.
...
6. Fix blockers and majors by re-delegating to `geeky-coder` once, then re-run validation and review. If the blocker or major finding is not resolved after one follow-up, move the task to Blocked and stop.
...
If a gate fails, fix mutable artifacts or implementation issues and re-run the same gate once after each mutable-artifact fix. If the gate still fails after one fix-and-retry cycle, treat this as a blocking gate failure per the Failure Handling section and stop. Never fix a failed DoD by editing frozen task bodies.
